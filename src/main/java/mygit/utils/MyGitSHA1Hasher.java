...
public class MyGitSHA1Hasher implements MyGitHasher {
     ...
     */
    @Override
    public @NotNull String getHashFromObject(@NotNull Object object) throws IOException {
        return bytesToHex(getByteHashFromObject(object));
    }

    /**
     * Splits the hash computed by {@link #getHashFromObject(Object)} into two parts:
     * directory hash will be directoryHash {@value mygit.utils.MyGitSHA1Hasher.SHA1HashParts#DIRECTORY_HASH_LENGTH} letters and the latter will serve as a filename hash
     * @param hash hash to split computed by {@link #getHashFromObject(Object)}
     * @return splitted hash
     * @throws MyGitIllegalArgumentException if the given hash length is not {@value mygit.utils.MyGitSHA1Hasher#HASH_LENGTH} letters
     */
    @Override
    public @NotNull MyGitHasher.HashParts splitHash(@NotNull String hash) throws MyGitIllegalArgumentException {
    ...
    private static byte[] getByteHashFromObject(@NotNull Object object) throws IOException {
        ...
        try (DigestOutputStream digestOutputStream = new DigestOutputStream(dummyOutputStream, messageDigest);
